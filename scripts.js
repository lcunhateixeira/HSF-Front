...
const getList = async () => {
  ...
  fetch(url, {
    ...
    .then((data) => { data.ubses.forEach(ubs => insertList(ubs.cnes, ubs.nome_fantasia, ubs.endereco, ubs.cidade, ubs.latitude, 
...
const novaUBS = () => {
  ...
  if (nome_fantasia === '' || endereco === '', cidade === '') {
    ...
    postUBS(cnes, nome_fantasia, endereco, cidade, latitude, longitude, telefone)
...
/*
  ...
  ------------------------------------------------------------------------------------------
*/
const postUBS = async (cnes, nome_fantasia, endereco, cidade, latitude, longitude, telefone) => {
  const formData = new FormData();
  formData.append('cnes', cnes);
  formData.append('nome_fantasia', nome_fantasia);
  formData.append('endereco', cidade);
...
const simulaMovimentoNasUBSes = () => {
  for (let i = 0; i < 100; i++) {
    let usuario_id = usuarios[numeroAleatorio(1, 1000)];
    let ubs_id = ubses[numeroAleatorio(1, 18)];
    postRegistro(usuario_id, ubs_id);
    getList();
    TimeRanges.sleep(3000);
  }
}
...
const insertList = (cnes, nome, endereco, latitude, longitude, telefone, fila, movimentacao_mes, media_movimentacao_diaria) => {
  ...
  createA.setAttribute('href', 'https://www.google.com/maps/@'+latitude+','+longitude);
